...
from run_pretrained_unet_inference import MODEL_FNAMES, UNET_FOLDER, download_model
...
def cli_main(args):
    ...
    # model
    # ------------
    model = UnetCustomLossModule(
        loss_function=SSIM(window_size=31, reduction='mean'),
    ...
    unet_model = Unet(in_chans=1, out_chans=1, chans=256, num_pool_layers=4, drop_prob=0.0)
    ...
    unet_model.load_state_dict(torch.load(state_dict_file))
...
def build_args():
    ...
    default_root_dir = fetch_dir("log_path", path_config) / "unet" / "unet_demo_ssim"
    ...
    # module config
    parser = UnetCustomLossModule.add_model_specific_args(parser)
    parser.set_defaults(
    ...
    args.callbacks = [
        pl.callbacks.ModelCheckpoint(
            ...
            mode="min",
            filename='{epoch}-{ssim:.2f}-{nmse:.2f}'
        )
    ]
