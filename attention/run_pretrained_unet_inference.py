...
MODEL_FNAMES = {
    ...
    #"unet_attention": "unet-attention-dict.pt",
}
...
def run_inference(challenge, state_dict_file, data_path, output_path, device):
    model = UnetAttention(in_chans=1, out_chans=1, chans=256, num_pool_layers=4, drop_prob=0.0)
    ...
    state_dict_file = 'unet-attention-dict.pt'
    ...
    ssim_function = SSIM(window_size=31, reduction='mean')

    for batch in tqdm(dataloader, desc="Running inference"):
        with torch.no_grad():
            output, slice_num, fname = run_unet_model(batch, model, device)
            loss = F.l1_loss(output, batch.target)
            loss_ssim = ssim_function(output.unsqueeze(0), batch.target.unsqueeze(0))
            losses.append({"l1_loss": loss.item(), "loss_ssim": loss_ssim.item()})
    ...
    fname = "loss_pretrained_" + str(datetime.datetime.now()) + ".json"
